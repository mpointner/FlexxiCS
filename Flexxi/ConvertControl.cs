...
namespace Flexxi
{
    public partial class ConvertControl : EditControlVorlage
    {
        ...
        private readonly List<string> ImageExtensions = new List<string> { "JPG", "JPEG", "GIF", "PNG" };
        ...
        delegate string RunCallback(string path);

        public string run(string path)
        {
            if (ConvertList.InvokeRequired)
            {
                return (string)this.Invoke(new RunCallback(run), new object[] { path });
            }
            else
            {
                modified = false;
        ...
        private void ConvertControl_SizeChanged(object sender, EventArgs e)
        {
            sizeChanged();
        }
    }
